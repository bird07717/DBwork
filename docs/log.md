...
## 2026-06-06 管理员登录页预览卡片重影修复

- 任务：修复管理员登录页左侧“访问方式 / 平台状态”两个半透明卡片出现多层历史文本上下错位重叠的 Ghosting 问题。
- 变更：保持登录页原有全局侧边栏布局和透明卡片视觉不变；移除预览卡片上的 `backdrop-filter` 模糊合成效果，改用半透明背景和独立绘制边界；为卡片增加 `isolation`、`contain: paint`、独立文本层级，并将彩色装饰线移到卡片上方，避免浏览器合成层缓存导致历史文本重影。
- 检查：`cd frontend && npm run build` 通过，保留既有 Vite chunk 体积警告。
- 结果：通过，等待用户浏览器复测登录页左侧预览卡片重影。
